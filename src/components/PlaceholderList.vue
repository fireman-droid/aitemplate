...
import placeholderMapping from '@/templates/house-sale-dispute/placeholders.json'
...
const emit = defineEmits(['fill-placeholders', 'preview-update'])
...
const isCheckboxField = (name) => {
  return name.includes('gender_') || name.includes('_yes') || name.includes('_no') ||
    name.includes('_has') ||
    name.includes('_none') ||
