          ...
          >
            ...
            <template v-for="subgroup in group.groups" :key="subgroup.id">
              <div class="subgroup-container">
                ...
                <!-- 如果有三级子分组，继续渲染 -->
                <template v-if="subgroup.sub_groups && subgroup.sub_groups.length > 0">
                  <div
                    v-for="thirdLevel in subgroup.sub_groups"
                    :key="thirdLevel.label"
                    class="third-level-container"
                  >
                    <div class="third-level-header">
                      <h5>{{ thirdLevel.label }}</h5>
                    </div>
...
const groups = computed(() => {
  if (!props.templateConfig || !props.templateConfig.template_config) {
    console.log('DynamicForm: 没有模板配置')
    return []
  }
  const sections = props.templateConfig.template_config.root_sections || []
  console.log('DynamicForm: 加载了', sections.length, '个分组')
  return sections
})
